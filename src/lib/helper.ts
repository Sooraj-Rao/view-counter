...
import { ColorStyleKey, SVGOptions } from "./types";
...
export const SendOption = (searchParams: URLSearchParams) => {
  return {
...
export function generateSVG(
  ...
  const baseStyle = colorStyles[options.colorStyle as number] || colorStyles[1];
  const style = {
    ...baseStyle,
  ...
  const isGradientTheme = parseInt(options.colorStyle as string) > 10;
